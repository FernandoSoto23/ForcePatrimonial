...
import CasoCriticoModal from "./components/CasoCriticoModal";
import ActivoCard from "./components/ActivoCard";
import CriticoCard from "./components/CriticoCard";
const MapaUnidadLive = lazy(() => import("./components/MapaUnidadLive"));
/* ============================================================
...
export default function Casos() {
            ...
            <div className="mt-3 border-t pt-3 text-xs space-y-3">
              {c.eventos.map((e, i) => (
                <div key={`${e.id ?? "noid"}-${i}`} className="pt-2">
                  <strong className="text-gray-900">{normalize(e.tipo)}</strong>
  ...
  return (
    <>
      ...
      <div className="relative z-10 p-4 mt-10 pointer-events-none">
        <div
          className="
    grid
    grid-cols-1
    lg:grid-cols-[360px_360px]
    gap-4
    pointer-events-auto
    max-w-[320px]
  "
        >
          {casoCriticoSeleccionado && (
            <CasoCriticoModal
              caso={casoCriticoSeleccionado}
              onClose={() => setCasoCriticoSeleccionado(null)}
              protocolosEjecutados={protocolosEjecutados}
              marcarProtocolo={marcarProtocolo}
              detalleCierre={detalleCierre}
              setDetalleCierre={setDetalleCierre}
              hayAlMenosUnProtocoloEjecutado={hayAlMenosUnProtocoloEjecutado}
              resetearProtocolos={resetearProtocolos}
              cerrarModalYEliminarCaso={cerrarModalYEliminarCaso}
              usuario={usuario}
              API_URL={API_URL}
              toast={toast}
              normalize={normalize}
              ...
              ProtocolLauncher={ProtocolLauncher}
              ProtocoloAsaltoUnidadUI={ProtocoloAsaltoUnidadUI}
              FlowRunnerBotonPanico={FlowRunnerBotonPanico}
              ProtocoloDesvioRutaNoAutorizadoUI={
                ProtocoloDesvioRutaNoAutorizadoUI
              }
              ProtocoloEnfermedad={ProtocoloEnfermedad}
              InseguridadSinRiesgo={InseguridadSinRiesgo}
              UnidadDetenida={UnidadDetenida}
              UnidadSinSenal={UnidadSinSenal}
            />
          )}
              ...
              {activos.map((c) => (
                <ActivoCard
                  ...
                  onToggle={(caso) =>
                    setCasos((p) => ({
                      ...p,
                      [caso.id]: { ...caso, expanded: !caso.expanded },
                    }))
                  }
                  onAnalizar={setCasoSeleccionado}
                  onMapa={(caso) =>
                    setMapaUnidad({
                      unitId: caso.unitId,
                      unidad: caso.unidad,
